...
class CosineIncrementalNet(BaseNet):
    ...
    def generate_fc(self, in_dim, out_dim):
        ...
        if self.fc is None:
            fc = CosineLinear(in_dim, out_dim*offset, self.nb_proxy, to_reduce=True)
        else:
            prev_out_features = self.fc.out_features // self.nb_proxy
            
            if self.args["ssla_tasks"] == "init_only":
                fc = SplitCosineLinear(
                    in_dim, prev_out_features, out_dim + self.args["init_cls"]*(offset-1) - prev_out_features, self.nb_proxy
                )
            else:
                fc = SplitCosineLinear(
                    in_dim, prev_out_features, out_dim * offset - prev_out_features, self.nb_proxy
                )
